...
def normalize(data):
	return data.resample(rule='5min').mean().bfill()
...
def run():
	...
	df_previo = df[:new_length]
	df_real = df[new_length:]
	...
	if(df_prediccion.mean()['requests'] >= df_prediccion.mean()['Holt_Winter'] and p_value_mean < alpha):
		veces = df_prediccion.mean()['requests']/df_prediccion.mean()['Holt_Winter']
	...
	elif (df_prediccion.mean()['requests'] <= df_prediccion.mean()['Holt_Winter'] and p_value_mean < alpha):
		veces = df_prediccion.mean()['Holt_Winter']/df_prediccion.mean()['requests']
